...
static const char* TAG = "ESP32ServoX";

ServoX::ServoX() {  // initialize this channel with plausible values, except pin # (we set pin # when attached)
...
ESP32PWMX* ServoX::getPwm() {

  return &pwm;
}

int ServoX::attach(int pin) {

  return (this->attach(pin, DEFAULT_uS_LOW, DEFAULT_uS_HIGH));
}

int ServoX::attach(int pin, int min, int max) {
...
if(
				...
				ESP_LOGE(TAG, "This pin can not be a servo: %d ServoX available on: 1-21,26,33-42", pin);
#elif defined(CONFIG_IDF_TARGET_ESP32S3)
    ESP_LOGE(TAG, "This pin can not be a servo: %d ServoX available on: 1-21,35-45,47-48", pin);
#elif defined(CONFIG_IDF_TARGET_ESP32C3)
    ESP_LOGE(TAG, "This pin can not be a servo: %d ServoX available on: 1-10,18-21", pin);
#else
    ESP_LOGE(TAG, "This pin can not be a servo: %d ServoX available on: 2,4,5,12-19,21-23,25-27,32-33", pin);
#endif
            return 0;
  }
#endif
...
int ServoX::readTicks() {
  return this->ticks;
}

bool ServoX::attached() {
  return (pwm.attached());
}

void ServoX::setTimerWidth(int value) {
...
int ServoX::readTimerWidth() {
  return (this->timer_width);
}

int ServoX::usToTicks(int usec) {
  return (int)((double)usec / ((double)REFRESH_USEC / (double)this->timer_width_ticks) * (((double)REFRESH_CPS) / (double)PWM_FREQUENCY));
}

int ServoX::ticksToUs(int ticks) {
  return (int)((double)ticks * ((double)REFRESH_USEC / (double)this->timer_width_ticks) / (((double)REFRESH_CPS) / (double)PWM_FREQUENCY));
}

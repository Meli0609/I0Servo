...
int ESP32PWMX::PWMCount = -1;  // the total number of attached servos
bool ESP32PWMX::explicateAllocationMode = false;
ESP32PWMX* ESP32PWMX::ChannelUsed[NUM_PWM];  // used to track whether a channel is in service
long ESP32PWMX::timerFreqSet[4] = { -1, -1, -1, -1 };
int ESP32PWMX::timerCount[4] = { 0, 0, 0, 0 };

static const char* TAG = "ESP32PWMX";
 ...
 *
 */
void ESP32PWMX::allocateTimer(int timerNumber) {
  if (timerNumber < 0 || timerNumber > 3)
    return;
  if (ESP32PWMX::explicateAllocationMode == false) {
    ESP32PWMX::explicateAllocationMode = true;
    for (int i = 0; i < 4; i++)
      ESP32PWMX::timerCount[i] = 4;  // deallocate all timers to start mode
  }
  ESP32PWMX::timerCount[timerNumber] = 0;
}
...
double ESP32PWMX::_ledcSetupTimerFreq(uint8_t pin, double freq,
                                      uint8_t bit_num) {
...
int ESP32PWMX::timerAndIndexToChannel(int timerNum, int index) {
  int localIndex = 0;
...
int ESP32PWMX::allocatenext(double freq) {
...
double ESP32PWMX::setup(double freq, uint8_t resolution_bits) {
...
void ESP32PWMX::adjustFrequencyLocal(double freq, double dutyScaled) {
...
void ESP32PWMX::adjustFrequency(double freq, double dutyScaled) {
  if (dutyScaled < 0)
...
double ESP32PWMX::writeTone(double freq) {
  for (int i = 0; i < timerCount[getTimer()]; i++) {
    int pwm = timerAndIndexToChannel(getTimer(), i);
    if (ChannelUsed[pwm] != NULL) {
      if (ChannelUsed[pwm]->myFreq != freq) {
...
double ESP32PWMX::writeNote(note_t note, uint8_t octave) {
...
double ESP32PWMX::readFreq() {
  return myFreq;
}
void ESP32PWMX::attach(int p) {
  pin = p;
  attachedState = true;
}
void ESP32PWMX::attachPin(uint8_t pin) {
...
void ESP32PWMX::attachPin(uint8_t pin, double freq, uint8_t resolution_bits) {
...
void ESP32PWMX::detachPin(int pin) {
#ifdef ESP_ARDUINO_VERSION_MAJOR
#if ESP_ARDUINO_VERSION >= ESP_ARDUINO_VERSION_VAL(3, 0, 0)
...
bool ESP32PWMX::checkFrequencyForSideEffects(double freq) {
...
ESP32PWMX* pwmFactory(int pin) {
  for (int i = 0; i < NUM_PWM; i++)
    if (ESP32PWMX::ChannelUsed[i] != NULL) {
      if (ESP32PWMX::ChannelUsed[i]->getPin() == pin)
        return ESP32PWMX::ChannelUsed[i];
    }
  return NULL;
}
